...
Test case 1:
Input: Test the possible errors that could occur with negative numbers. (-1, 1, -1, 2 -1, 2, 1, -1, 5)
...
Test case 2:
Input: (using fixed seed of 42) 1, 4, 1, 6, 5
...
Test case 3:
Input: (using fixed seed of 42) 2, 50, 4, 5
...
const int slotMoney[8]{100, 500, 1000, 0, 10000, 0, 1000, 500, 100};
...
/*
int PlinkoMachine(int userSelection, int numChips, int slotSelection) {
for (int i = 0; i < numChips; i++) {
double chipPath = 0.0;
double winnings = 0;
chipPath = slotSelection;
for (int yAxisPosition = Y_LIMIT; yAxisPosition >= 0; yAxisPosition--) {
if (chipPath == MIN_SLOT) {
...
winnings = winnings + ComputeWinnings(chipPath);
}
}
*/
int main() {
	...
	while (userSelection != QUIT_PROGRAM) {
		...
		if (userSelection == SINGLE_CHIP_PLAY) {
			...
			cout << "Winnings: $" << ComputeWinnings(slot) << endl;
			cout << endl;
		}
