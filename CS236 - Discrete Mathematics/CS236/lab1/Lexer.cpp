...
void Lexer::scanner(string fileName) {
...
string Lexer::enumToString(tokenType someEnum) {
  switch (someEnum) {
    case COMMA :
    return "COMMA";
    case PERIOD :
...
string Lexer::getTokenList() {
  stringstream ss;
  for (int i = 0; i < tokenList.size(); i++) {
    ss << "("  << enumToString(tokenList.at(i).getTokenType()) << ",\"" << tokenList.at(i).getString() << "\"," << tokenList.at(i).getLine() << ")" << endl;
  }
