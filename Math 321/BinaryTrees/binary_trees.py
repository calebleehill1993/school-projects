...
class SinglyLinkedList:
    ...
    def recursive_find(self, data):
        ...
        """
        def step(data, node):
            """Takes steps towards finding a matching node"""
            if node is not None:
                if node.value == data:
                    return node
                return step(data, node.next)
            raise ValueError(str(data) + " is not in the list.")
        return step(data, self.head)
...
class BST:
    ...
    def insert(self, data):
        ...
        Example:
            ...
            [8]                                 |
        """
        def find_parent(current, data):
            """Searches for the parent of a node that is being inserted to the BST."""
            if current.value == data:
                raise ValueError(str(data) + " is a duplicate")
                return
            elif data < current.value and current.left is not None:
                return find_parent(current.left, data)
            elif data > current.value and current.right is not None:
                return find_parent(current.right, data)
            return current
        if self.root is None:
            self.root = BSTNode(data)
            return
        parent = find_parent(self.root, data)
